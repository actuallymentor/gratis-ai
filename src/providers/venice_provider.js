/**
 * Venice LLM Provider — implements the LLMProvider interface for cloud inference.
 *
 * Uses Venice's OpenAI-compatible API for streaming chat completions.
 * No endpoint management, no warming, no GPU selection — just an API key and model ID.
 *
 * @module venice_provider
 */
import { log } from 'mentie'
import { venice_headers } from './venice_service'
 ...
 */
export default class VeniceProvider {

    /**
     * @param {Object} config
     * @param {string} config.api_key - Venice API key
     * @param {string} config.model_id - Venice model ID (e.g. `dolphin-2.9.2-qwen2.5-72b`)
     */
    ...
    /**
     * "Load" the model — Venice models are always available, no warming needed.
     */
    async load_model() {

        log.info( `[venice] Activating model ${ this._model_id }` )
        this._ready = true
    ...
    async chat( messages, opts = {} ) {
        ...
        const res = await fetch( `${ VENICE_BASE }/chat/completions`, {
            method: `POST`,
            headers: venice_headers( this._api_key ),
    ...
    /**
     * Unload — no cleanup needed for Venice.
     */
    async unload_model() {
        this._ready = false
    }
     ...
     */
    get_loaded_model() {
        return this._ready ? `venice:${ this._model_id }` : null
    }
...
/**
 * Extract a human-readable error message from a Venice API error response.
...
function _format_api_error( status, body ) {
    ...
    return `Venice error (${ status }): ${ body || `Unknown error` }`
